...
class HeritageDatabaseTest {
    private lateinit var heritageDao: HeritageDao
    private lateinit var heritageDatabase: HeritageDatabase

    @Before
    fun createDb() {
        val context = ApplicationProvider.getApplicationContext<Context>()
        heritageDatabase = Room.inMemoryDatabaseBuilder(
            context, HeritageDatabase::class.java
        ).build()
        heritageDao = heritageDatabase.heritageDao()
    }

    @After
    @Throws(IOException::class)
    fun closeDb() {
        heritageDatabase.close()
    }

    @Test
    @Throws(Exception::class)
    fun givenAListOfHeritageEntity_itShouldInsertThemAllAndThenRetrieveThem() {
        runBlocking {
            val expectedHeritageEntityList = listOf(
                HeritageEntity(
                    id = 1,
                    jsonId = "3",
                    year = 1978,
