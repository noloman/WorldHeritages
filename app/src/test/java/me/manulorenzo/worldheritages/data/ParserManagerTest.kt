...
class ParserManagerTest {
    lateinit var sut: ParserManager
    ...
    @Test
    fun `given a list of heritages from a JSON file, it should return a list of HeritageEntity`() =
        runBlocking {
            doAnswer {
                ByteArrayInputStream(
                    fakeJsonList.toByteArray(Charsets.UTF_8)
                )
            }.whenever(mockAssetManager).open(any())
            val heritageList = sut.parseAssetsToEntityList()
            assertTrue(heritageList.size == 1)
            assertEquals(heritageList, Faker.fakeHeritageEntityList)
        }
}